...
class GetMedMNIST(Dataset):
    def __init__(self, name='BloodMNIST', split='train', transform=transforms.ToTensor(),
                 download=True, size=224, verbose=True):
        """
        ...
        if verbose:
            ...
            print(f'Chosen split: {self.split}\n')
            print(f'Classes:')
            for i,l  in self.info['label'].items():
                print(f'\t{i:2s} : {l:70} n data points: {getattr(self, f"{i}_len")}')
            print(f'\n{textwrap.fill(self.info["description"], width=85)}\n')
